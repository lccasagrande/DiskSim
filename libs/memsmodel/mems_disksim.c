...
void mems_get_mapping(int maptype, int devno, int blkno,
...
void mems_get_average_position(tipsector_coord_set_t *up,
                                       ...
                                       int *numptr)
{
  ...
  result = (mems_t *)malloc(sizeof(mems_t));
  if (!result)
    return 0;
  ...
  if (disksim->memsinfo == NULL)
  {
    disksim->memsinfo = (struct mems_info *)malloc(sizeof(struct mems_info));
  ...
  {
    int newlen = disksim->memsinfo->devices_len ? disksim->memsinfo->devices_len * 2 : 2;
...
struct device_header *mems_copy(struct device_header *src)
{
  mems_t *result = (mems_t *)calloc(1, sizeof(mems_t));
  memcpy(result, src, sizeof(mems_t));
  result->queue = ioqueue_copy(((mems_t *)src)->queue);
  memcpy(&result->sled, ((mems_t *)src)->sled, ((mems_t *)src)->num_sleds * sizeof(mems_sled_t));
...
void mems_initialize(void)
{
  ...
  for (i = 0; i < disksim->memsinfo->devices_len; i++)
  {
    ...
    if (dev)
    {
      ...
      if (dev->numsegs == 0)
        addlisttoextraq((event **)&dev->seglist);
...
double mems_unimplemented(int diskno, ioreq_event *req, double maxtime)
{
